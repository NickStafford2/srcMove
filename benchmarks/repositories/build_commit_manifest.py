...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a readable ordered commit manifest for a repository benchmark."
    )
    parser.add_argument(
        "case",
        help="Case name under benchmarks/repositories, e.g. wowy_advanced_analytics.",
    )
    parser.add_argument(
        "--branch",
        default="HEAD",
...
def main() -> int:
    ...
    if not repo_dir.is_dir():
        ...
        print(
            "Run run_case.py or build_examples.py once first so the repo is cloned.",
            file=sys.stderr,
        )
        return 1
