...
def require_safe_target(target: Path) -> Path:
    ...
    if not is_relative_to(SCRIPT_PATH, BENCHMARK_ROOT):
        raise RuntimeError(
            f"script must live under the repository benchmark directory: {SCRIPT_PATH}"
        )

    if not is_relative_to(resolved, BENCHMARK_ROOT):
        raise RuntimeError(
            f"refusing to delete files outside the repository benchmark directory: {resolved}"
        )
