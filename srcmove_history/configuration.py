...
def load_history_configuration(analysis_root: Path) -> HistoryConfiguration:
    ...
    try:
        ...
        raise ValueError(f"srcmove-history configuration is unreadable: {path}") from error
    if not isinstance(value, dict):
        raise ValueError(f"srcmove-history configuration must be a TOML table: {path}")
    ...
    if (
        ...
        raise ValueError(
            f"unsupported srcmove-history configuration schema: "
            f"{schema_version!r}"
        )
...
def create_history_configuration(
    ...
    if path.exists() or path.is_symlink():
        raise ValueError(f"srcmove-history configuration already exists: {path}")
