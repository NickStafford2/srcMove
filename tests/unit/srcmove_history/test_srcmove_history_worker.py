...
from srcmove_history.contracts import PairStatus, PairWorkItem
from srcmove_history.coordinator import run_pairs
from srcmove_history.git import GitBatch, GitMaterializationError
from srcmove_history.process import (
    run_process,
    validate_xml_artifact,
)
from srcmove_history.worker import PairExecutor, _remove_tree_within
...
class PairExecutorTests(unittest.TestCase):
    ...
    def test_publication_acknowledgement_removes_worker_scratch(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(
                list(analysis.glob("srcmove-history-worker-*")), []
            )
    ...
    def test_worker_reuses_one_git_batch_and_runs_tools_in_order(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with mock.patch("srcmove_history.worker.GitBatch", CountingBatch):
                run_pairs(
                    work,
